            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                {data.map(item => (
                    ...
                    >
                        ...
                        <h4 className="text-sm text-gray-500 mb-1">Category: {item.category}</h4>
                        <h3 className="text-lg font-semibold text-gray-800 mb-2 text-center">{item.title}</h3>
                        <p className="text-gray-700 mb-1">Remaining: {item.stock}</p>
                        <p className="text-gray-700 mb-1">Price: ${item.price}</p>
                        <p className={`text-sm font-medium ${item.availabilityStatus === 'In Stock' ? 'text-green-600' : 'text-red-600'}`}>
